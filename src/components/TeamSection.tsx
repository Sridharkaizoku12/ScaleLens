...
export function TeamSection({ startupId }: { startupId: string }) {
  ...
  const handleAdd = async () => {
    ...
    setName(""); setEmail(""); setRole(""); setOpen(false); setSaving(false);
    refetchTeam();
  };
  ...
  return (
    <Card className="col-span-1 md:col-span-2 lg:col-span-3">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button size="sm" onClick={() => setOpen(true)}>+ Add Teammate</Button>
      </CardHeader>
      <CardContent>
        {loading ? <div>Loading...</div> : (
          <ul className="space-y-2">
            ...
            {team.map((member) => (
              <li key={member.id} className="border rounded p-2 flex flex-col md:flex-row md:items-center md:gap-4">
